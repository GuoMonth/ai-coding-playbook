...
这个选择不是为了教授 React，也不是说它适合所有业务。它只是本教程的默认练习环境：启动快、页面反馈直观、AI coding 工具熟悉，也便于后续逐步加入测试。

学习者前期不需要选择技术方案。先按默认方案走完整个反馈闭环，等理解了规格、验证、测试和改动影响之后，再讨论是否换成自己的团队技术栈。

测试也会逐步引入：前几章以页面可见和人工验证为主，中期加入简单自动化测试，后期再讨论更完整的测试和回归策略。
